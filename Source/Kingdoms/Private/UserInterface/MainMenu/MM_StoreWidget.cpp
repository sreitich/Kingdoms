...
void UMM_StoreWidget::DeactivateWidget()
{
...
void UMM_StoreWidget::NativeConstruct()
{
	Super::NativeConstruct();

	/* Bind the "back" button to deactivate this widget. */
	BackButton->OnClicked.AddDynamic(this, &UMM_StoreWidget::OnBackClicked);
	...
	AnimBinding.BindDynamic(this, &UMM_StoreWidget::OnDeactivatedAnimEnd);
	BindToAnimationFinished(OnDeactivatedAnim, AnimBinding);
}

void UMM_StoreWidget::OnDeactivatedAnimEnd()
{
	...
	HUDPtr->ChangeMenus();
	HUDPtr->CreateStoreWidget(true);
}

void UMM_StoreWidget::OnBackClicked()
{
